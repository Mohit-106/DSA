...
void multisolver(vector<Edge> graph[], int src, int dst, vector<bool> visited, int criteria, int k, string psf, int wsf)
{
    ...
    for (Edge v : graph[src])
    {
        ...
        {
            string ans = to_string(v.nbr);
            int ans2 = v.wt;
            multisolver(graph, v.nbr, dst, visited, criteria, k, psf + ans, wsf + ans2);
        }
    }
    visited[src] = false;
}
